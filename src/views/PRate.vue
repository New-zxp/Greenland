<template>
  <div ref="container" style="position: relative; left: 620px; top: 630px; width: 280px;"></div>
</template>
...
export default {
    mounted() {
        ...
        const intervals = [40, 4000];
        let gradient = 'linear-gradient(to right, ';
        const label = document.createElement('div');
        label.innerHTML = 'P Rate cm/year';
        label.style.textAlign = 'center';
        label.style.fontSize = "15px"
        this.$refs.container.appendChild(label);
        const colorbar = document.createElement('div');
        colorbar.style.width = '280px';
        colorbar.style.height = '18px';
        this.$refs.container.appendChild(colorbar);
