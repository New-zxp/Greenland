...
function downloadData() {
    Promise.all([
        fetch('./src/data/download/no_GRACE_mass.csv').then(response => response.text()), // 使用 response.text() 解析 CSV 数据
        fetch('./src/data/download/no_MBM_mass.csv').then(response => response.text()) // 使用 response.text() 解析 CSV 数据
        ...
        const graceDataFileName = 'no_GRACE_mass.csv';
        const mbmDataFileName = 'no_MBM_mass.csv';
...
Promise.all([
    fetch('./src/data/no_Grace_mass.json').then(response => response.json()),
    fetch('./src/data/no_MBM_mass.json').then(response => response.json())
        ...
        series: [
            ...
            },
            {
                ...
                labelLayout(params) {
                    return {
                        x: params.rect.x - 20,
                        y: params.rect.y + params.rect.height / 0.9,
            ...
            },
            {
                ...
                labelLayout(params) {
                    return {
                        x: params.rect.x + 130,
                        y: params.rect.y + params.rect.height / 3.4,
                        verticalAlign: 'middle',
                        align: 'left'
                    }
                },
