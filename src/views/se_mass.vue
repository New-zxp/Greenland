...
function downloadData() {
    Promise.all([
        fetch('./src/data/download/se_GRACE_mass.csv').then(response => response.text()), // 使用 response.text() 解析 CSV 数据
        fetch('./src/data/download/se_MBM_mass.csv').then(response => response.text()) // 使用 response.text() 解析 CSV 数据
        ...
        const graceDataFileName = 'se_GRACE_mass.csv';
        const mbmDataFileName = 'se_MBM_mass.csv';
...
Promise.all([
    fetch('./src/data/se_Grace_mass.json').then(response => response.json()),
    fetch('./src/data/se_MBM_mass.json').then(response => response.json())
    ...
    myChart.setOption({
        ...
        yAxis: {
            ...
            nameGap: 50,
            data: ["-1200", "-900", "-600", "-300", "0", "300", "600"],
            min: -1200,
            max: 600,
            axisLabel: {
                fontFamily: 'Arial', // 设置字体
        ...
        series: [
            ...
            },
            {
                ...
                labelLayout(params) {
                    return {
                        x: params.rect.x + 130,
                        y: params.rect.y + params.rect.height / 3.2,
                        verticalAlign: 'middle',
                        align: 'left'
                    }
                },
