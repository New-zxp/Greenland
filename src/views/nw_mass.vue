...
function downloadData() {
    Promise.all([
        fetch('./src/data/download/nw_GRACE_mass.csv').then(response => response.text()), // 使用 response.text() 解析 CSV 数据
        fetch('./src/data/download/nw_MBM_mass.csv').then(response => response.text()) // 使用 response.text() 解析 CSV 数据
        ...
        const graceDataFileName = 'nw_GRACE_mass.csv';
        const mbmDataFileName = 'nw_MBM_mass.csv';
        ...
        series: [
            ...
            },
            {
                ...
                labelLayout(params) {
                    return {
                        x: params.rect.x + 126,
                        y: params.rect.y + params.rect.height / 2.8,
                        verticalAlign: 'middle',
                        align: 'left'
                    }
                },
